...
interface LocoBookingListProps {
  bookings: RawBooking[];
  locos: Loco[];
  jobs: Job[];
  locoTypes: LocoType[];
...
const LocoBookingList: React.FC<LocoBookingListProps> = ({
  bookings,
  locos,
  listSearch,
  setListSearch,
  ...
  const filteredTodayBookings = bookings.filter((b) => {
    const ml = locos.find((l) => l.loco_number === b.loco_number);
    const matchesSearch = b.loco_number.toLowerCase().includes(listSearch.trim().toLowerCase());
      ...
      <div className="panel-card scrollable">
        ...
        <div className="timeline-grouped-bookings">
          ...
          {filteredTodayBookings.length === 0 && (
            <p className="no-records-hint">No workshop operations booked for today.</p>
          )}
        </div>
      </div>
    </section>
...
export default LocoBookingList;
